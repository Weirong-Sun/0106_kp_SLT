"""
Inference script for hierarchical skeleton reconstruction
Load trained model and generate skeleton images from body keypoints
"""
import torch
import pickle
import numpy as np
...
def extract_representations(model, kp_dict, normalization=None, device='cuda'):
    """
    ...
    Returns:
        global_repr: Global representation [1, d_global]
        regional_repr: Regional representation [1, num_regions, d_region]
    """
    keypoints = prepare_keypoints(kp_dict, normalization, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoints_skeleton_hierarchical/best_model.pth')
    ...
    output_dir = "visualizations_skeleton_hierarchical"
    ...
    for idx in range(num_samples_to_process):
        ...
        global_repr, regional_repr = extract_representations(model, kp_dict, normalization, args.device)
        ...
        if idx == 0:  # Print representation info only for first sample
            ...
            print(f"Global repr - Mean: {global_repr.mean().item():.4f}, Std: {global_repr.std().item():.4f}")
            print(f"Regional repr - Mean: {regional_repr.mean().item():.4f}, Std: {regional_repr.std().item():.4f}")
